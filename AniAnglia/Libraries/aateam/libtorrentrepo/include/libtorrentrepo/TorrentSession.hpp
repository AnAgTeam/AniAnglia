#pragma once
#include "libtorrent_include.hpp"
#include "BaseError.hpp"

#include <queue>
#include <mutex>
#include <thread>
#include <condition_variable>
#include <exception>
#include <chrono>
#include <set>

namespace libtorrentrepo {

	struct TorrentSessionException : TorrentRepoError {
		virtual const char* what() const noexcept override;
	};
	struct TorrentTimoutException : TorrentSessionException {
		virtual const char* what() const noexcept override;
	};

	class TorrentSession;
	class TorrentItem;

	struct TorrentFileInfo {
		std::string name;
		int64_t size;
	};

	class TorrentFile {
		friend class TorrentItem;

	public:
		TorrentFile(std::mutex& buf_mutex, std::condition_variable& buf_cv, lt::torrent_handle& handle, const std::shared_ptr<const lt::torrent_info>& info, lt::file_index_t file_index, std::streamoff offset);

		// read n bytes. If -1 reads first available piece. block execution when piece not available
		std::string read(const std::streamsize n = -1);
		void seek(const std::streamoff offset, std::ios::seekdir way);
		std::streamoff tell() const noexcept;
		bool eof() const noexcept;

	protected:
		void piece_recieved(const boost::shared_array<char>& buffer, std::streamsize size, lt::piece_index_t piece);

	private:
		void _wait_for_piece() const;

		lt::file_index_t _file_index;
		lt::piece_index_t _first;
		lt::piece_index_t _last;
		lt::piece_index_t _end;
		lt::piece_index_t _current;
		std::streamoff _offset;
		int _foffset;
		int _coffset;
		int _eoffset;
		std::shared_ptr<const lt::torrent_info> _info;
		int64_t _size;
		lt::torrent_handle _handle;
		std::mutex& _cv_mutex;
		std::condition_variable& _cv;

		lt::piece_index_t _shared_buffer_piece;
		boost::shared_array<char> _shared_buffer;
		std::streamsize _shared_buffer_size;
	};

	class TorrentItem {
		friend class TorrentSession;

		static constexpr int RETRY_COUNT = 40;

	public:
		TorrentItem(const lt::torrent_handle& handle, const lt::info_hash_t& hash);

		std::vector<TorrentFileInfo> files();

		// syncronous
		TorrentFile& start_download(const lt::file_index_t& file_index);

		void set_timeout(const std::chrono::system_clock::duration& timeout);

	protected:
		void handle_alert(lt::alert* const a);

	private:
		void _wait_for_metadata();

		lt::info_hash_t _info_hash;
		lt::torrent_handle _handle;
		std::chrono::system_clock::duration _timeout;
		std::vector<TorrentFile> _downloads;

		std::mutex _cv_mutex;
		std::condition_variable _cv;
		lt::piece_index_t _shared_buffer_piece;
		boost::shared_array<char> _shared_buffer;
		std::streamsize _shared_buffer_size;
	};

	class TorrentSession {
		friend class TorrentItem;

	public:
		TorrentSession();
		~TorrentSession();

		TorrentItem* add_torrent(std::string_view magnet_url, std::string_view save_path, std::vector<std::string> trackers);
		void remove_torrent(std::string_view magnet_url);

		void add_default_tracker(std::string_view tracker);

	private:
		void _handle_alerts();

		std::vector<std::string> _trackers;
		std::vector<std::shared_ptr<TorrentItem>> _torrent_items;
		lt::session _session;
		std::thread _alert_handle_thread;
		bool _alert_recieved;
		std::condition_variable _cv;
		std::mutex _mutex;
		bool _destructing;
	};

};

