...
namespace libanixart {
	...
	template<typename TCode, typename TError>
	void assert_status_code(const int32_t& code) {
		auto code_e = static_cast<TCode>(code);
		if (code_e != TCode::Success) {
			throw TError(code_e);
		}
	}
	template<typename TCode, typename TError>
	void assert_status_code(JsonObject& object) {
		assert_status_code<TCode, TError>(json::ParseJson::get<int32_t>(object, "code"));
	}
	...
	using CollectionResponseError = ApiCodeError <codes::collection::CollectionResponseCode, ApiCollectionRequestError >;
	using CreateEditCollectionError = ApiCodeError<codes::collection::CreateEditCollectionCode, ApiCollectionRequestError>;
	using DeleteCollectionError = ApiCodeError<codes::collection::DeleteCollectionCode, ApiCollectionRequestError>;
	...
	using FavoriteCollectionDeleteError = ApiCodeError<codes::collection::FavoriteCollectionDeleteCode, ApiCollectionRequestError>;
	...
	using CollectionCommentDeleteError = ApiCodeError<codes::collection::comment::CollectionCommentDeleteCode, ApiCollectionRequestError>;
	...
	using DeleteVoteReleaseError = ApiCodeError<codes::release::DeleteVoteReleaseCode, ApiReleaseRequestError>;
